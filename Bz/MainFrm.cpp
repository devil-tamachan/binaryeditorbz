...
BOOL CMainFrame::CreateClient(CCreateContext* pContext) 
{
	...
	if(bReCreate) {
		...
		if(m_pSplitter) {
			if(m_pSplitter->GetRowCount()>1 && m_pSplitter->GetColumnCount()>1) {
				...
				m_pSplitter->CreateView(0,1,RUNTIME_CLASS(CWnd), CSize(0,0), pContext);//MFCXXね
				m_pSplitter->SetActivePane(0,1);//MFCXXね
				...
				m_pSplitter->CreateView(m_pSplitter->GetRowCount() - 1, m_pSplitter->GetColumnCount() - 1,RUNTIME_CLASS(CWnd), CSize(0,0), pContext);//MFCXXね
				m_pSplitter->SetActivePane(m_pSplitter->GetRowCount() - 1, m_pSplitter->GetColumnCount() - 1);//MFCXXね
				m_pSplitter->DeleteView(0,0);
			}
	...
	if((m_bStructView||m_bInspectView) && m_nSplitView != ID_VIEW_SPLIT_V) {
		m_pSplitter->SetColumnInfo(0, options.xSplitStruct, 0);
	}
...
void CMainFrame::GetSplitInfo()
{
	...
	case ID_VIEW_SPLIT_V:
		...
		options.xSplit = nCur;
		if(m_bBmpView || m_bStructView || m_bInspectView) {
	...
	if(m_bStructView || m_bInspectView) {
		m_pSplitter->GetColumnInfo(0, nCur, nMin);
