...
//			m_pFileMapping = pFile;
	...
	TAMADataChunk* _OldFileMap_Conv2TAMADataChunk(TAMAOLDFILECHUNK *pOldFileChunk, DWORD *pNRemain)
	{
		...
		case OF_FD:
		{
			TAMADataBuf *pTAMADataBuf = _TAMADataBuf_CreateAssign(pMem, 0);
			ASSERT(pTAMADataBuf);
...
// Read/OverWrite
	...
	BOOL Write(DWORD dwStart, void *srcDataDetached/*Write()失敗した場合は呼び出し元で開放すること*/, DWORD dwSize/*, BOOL bNotCopy = FALSE*/)
	{
		...
		return _FileMap_OverWriteTAMADataChunks(dwStart, undo->dataNext, undo->nDataNext, 0);
	}
...
// Insert/Delete
	...
	BOOL Delete(DWORD dwDelStart, DWORD dwDelSize)
	{
		...
		DWORD nDataChunks = 0;
		BOOL bRet = _FileMap_CreateTAMADataChunks(dwDelStart, dwSize, &pDataChunks, &nDataChunks);
		if(!bRet)
		{
			ASSERT(FALSE);
			free(pNewUndo);
