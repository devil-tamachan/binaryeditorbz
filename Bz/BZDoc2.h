...
public:
  ...
  BOOL Write(void *pSrcData, DWORD dwStart, DWORD dwSize) { return m_pSFC ? m_pSFC->Write(pSrcData, dwStart, dwSize) : FALSE; }
  BOOL Insert(void *pSrcData, DWORD dwInsStart, DWORD dwInsSize) { return m_pSFC ? m_pSFC->Insert((LPBYTE)pSrcData, dwInsStart, dwInsSize) : FALSE; };
  BOOL Delete(DWORD dwDelStart, DWORD dwDelSize) { return m_pSFC ? m_pSFC->Delete(dwDelStart, dwDelSize) : FALSE; }
  BOOL Fill(LPBYTE pData, DWORD dwDataSize, DWORD dwStart, DWORD dwFillSize) { return m_pSFC ? m_pSFC->Fill(pData, dwDataSize, dwStart, dwFillSize) : FALSE; }

  BOOL DoUndo(DWORD *pRetStart = NULL) { return m_pSFC ? m_pSFC->Undo(pRetStart) : FALSE; }
